    ...
    public void run() {
            ...
            try {
                while (true) {
                    ...
                    //判断登录/注册信息
                    if(m.getMessageType()==Data.RegInfo){
                        ...
                        try {
                            if(parkService.parkRegister(p)==1) {
                                ...
                                System.out.println("正常设置注册失败");
                                m = new Message();
                                m.setStatu(false);
                                oos.writeObject(m);
                            }
                        } catch (ParkException pe) {
                            System.out.println("异常设置注册失败");
                            m = new Message();
                            m.setStatu(false);
                            oos.writeObject(m);
                        }
                        ...
                        //登录处理
                        if (m.getMessageType()==Data.Login) {
                            try {
                                if((SocketThreadManage.socketThread.get(m.getParkId())==null)&&parkService.parkLogin(m.getParkId(),m.getPassword())==1) {
                                    List<ParkInfo> l = parkService.getParksInfoList();
                                    Message message = new Message();
                                    for(int i=0;i<l.size();i++) {
                                        if(l.get(i).getParkid()==m.getParkId()) {
                                            message.setParkName(l.get(i).getName());
                                        }
                                    }
